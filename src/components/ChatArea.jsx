...
import { LuCalendarRange, LuCalendarDays, LuCircleMinus, LuCirclePlus } from "react-icons/lu"
...
export default function ChatArea({ selected }) {
  ...
  // Control de chips en móvil: mostrar solo uno y botón "Más"
      ...
      <div>
        <div ref={setContainerEl} className="flex-1 flex flex-col min-h-0 relative overflow-x-hidden" style={{ height: '100dvh' }}>
          ...
          >
            ...
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-start md:justify-center py-6 md:py-10 mt-96 md:mt-100">
                ...
                {/* Chips: móvil con 'Más' y scroll horizontal; desktop como antes */}
                {isMobile ? (
                  !chipsExpanded ? (
                    <div className="w-full px-4 flex flex-col items-center gap-3">
                      {/* Solo primer chip visible */}
                      <button
                        onClick={() => sendQuestion(suggestions[0].text)}
                      ...
                      >
                        <span className="inline-flex items-center justify-center w-5 h-5">{suggestions[0].icon}</span>
                        <span className="text-sm">{suggestions[0].text}</span>
                      </button>
                      <button
                        type="button"
                        onClick={() => setChipsExpanded(true)}
                        className="inline-flex items-center gap-2 text-white/90 px-4 py-2 rounded-full border border-[#223555] bg-transparent hover:bg-white/5 transition-colors"
                        aria-label="Mostrar más sugerencias"
                      >
                        <LuCirclePlus className="w-4 h-4 text-white" />
                        <span className="text-sm">Más</span>
                      </button>
                    </div>
                  ) : (
                    <div className="w-full max-w-4xl mx-auto">
                      <div className="flex gap-3 overflow-x-auto px-4 -mx-4 py-1 whitespace-nowrap" style={{ WebkitOverflowScrolling: 'touch' }}>
                        {suggestions.map((s, idx) => (
                          <button key={idx} onClick={() => sendQuestion(s.text)} className="shrink-0 inline-flex items-center gap-2 text-white/90 px-4 py-2 rounded-full border border-[#223555] bg-transparent hover:bg-white/5 transition-colors">
                            <span className="inline-flex items-center justify-center w-5 h-5">{s.icon}</span>
                            <span className="text-sm">{s.text}</span>
                          </button>
                        ))}
                        {/* Cerrar/menos como píldora */}
                        <button
                          type="button"
                          onClick={() => setChipsExpanded(false)}
                          className="shrink-0 inline-flex items-center gap-2 text-white/90 px-4 py-2 rounded-full border border-[#223555] bg-transparent hover:bg-white/5 transition-colors"
                          aria-label="Ocultar sugerencias"
                        >
                          <LuCircleMinus className="w-4 h-4 text-white" />
